...
if [ $# -lt 2 ]; then
  ...
  echo "Example: ./deploy_github_pages.sh https://github.com/USER/calckosh.git https://USER.github.io/calckosh"
  exit 1
fi
REPO="$1"; PUBLIC="$2"
...
git init
git add .
git -c user.name="CalcKosh Deploy" -c user.email="deploy@localhost" commit -m "Deploy CalcKosh v3" || true
git branch -M main
